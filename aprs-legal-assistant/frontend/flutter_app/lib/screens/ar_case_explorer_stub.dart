...
class _ARCaseExplorerScreenStubState extends State<ARCaseExplorerScreenStub> {
  ...
  bool _useXTTS = true; // Use XTTS-v2 by default
  bool _xttsAvailable = false;
  ...
  Future<void> _checkXTTSAvailability() async {
    try {
      final available = await XTTSService.isServiceAvailable();
      setState(() {
        _xttsAvailable = available;
        _useXTTS = available; // Use XTTS if available
      });
      
      if (available) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('XTTS-v2 voice service connected!'))
        );
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> _speakAI(CourtroomPersona persona) async {
    ...
    final aiText = 'Sample AI response for ${persona.name} in the role of ${persona.role}. '
        'This is a demonstration of the ${_useXTTS ? "XTTS-v2" : "browser"} voice synthesis. '
    ...
    try {
      ...
      if (_useXTTS && _xttsAvailable) {
        // Use XTTS-v2 for high-quality voice
        try {
          setState(() {
            _responseText = 'Generating high-quality voice...';
          });
          
          final response = await XTTSService.generateSpeech(
            text: '${persona.name}: $aiText',
            personaRole: persona.role.toLowerCase(),
            speed: _ttsSpeed,
            temperature: 0.7,
          );
          
          // Play the audio
          _audioElement = XTTSService.playAudio(response.audioBase64);
          
          // Set up event listeners
          _audioElement?.onEnded.listen((_) {
            setState(() {
              _isLoading = false;
              _responseText = 'XTTS speech completed';
              _aiActive = false;
            });
          });
          ...
          _audioElement?.onError.listen((_) {
            setState(() {
              _isLoading = false;
              _responseText = 'XTTS playback error';
              _aiActive = false;
            });
          });
          
          setState(() {
            _responseText = 'Playing XTTS voice (${response.durationSeconds.toStringAsFixed(1)}s)';
          });
          
        } catch (e) {
          // Fall back to browser TTS if XTTS fails
          setState(() {
            _responseText = 'XTTS error: $e. Falling back to browser TTS.';
            _useXTTS = false;
          });
                                ...
                                children: [
                                  ...
                                  if (_xttsAvailable)
                                    Padding(
                                      ...
                                      child: Row(
                                        children: [
                                          const Text('Voice Engine:', style: TextStyle(color: Colors.white70)),
                                          const Spacer(),
                                          ChoiceChip(
                                            ...
                                            onSelected: (selected) {
                                              if (selected) {
                                                setState(() => _useXTTS = false);
                                              }
                                            },
                                          ),
                                          const SizedBox(width: 8),
                                          ChoiceChip(
                                            label: const Text('XTTS-v2'),
                                            selected: _useXTTS,
                                            onSelected: (selected) {
                                              if (selected) {
                                                setState(() => _useXTTS = true);
                                              }
                                            },
                                          ),
                                        ],
